...
export default function DeviceCheckModule() {
  ...
  const badge = {
    ...
    requesting: { text: 'Requesting access…',   color: '#111827' },
    granted:    { text: 'Devices Ready',        color: '#111827' },
    denied:     { text: 'Permission Blocked',   color: '#111827' },
    notfound:   { text: 'No Hardware Found',    color: '#111827' },
    busy:       { text: 'Camera In Use',        color: '#111827' },
    error:      { text: 'Device Error',         color: '#111827' },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'var(--font-inter)', backgroundColor: 'var(--bg-page)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 320px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', backgroundColor: '#0F172A', borderRight: '1px solid var(--border-color)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {status === 'denied' && (
            <CentreBox>
              <div style={{ backgroundColor: '#1E293B', border: '1px solid #111827', borderRadius: 12, padding: '24px 28px', maxWidth: 360, textAlign: 'center' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', backgroundColor: 'rgba(220,38,38,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px auto' }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#111827" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                </div>
                <h3 style={{ color: '#E5E7EB', fontSize: 15, fontWeight: 700, margin: '0 0 8px' }}>Permission Blocked</h3>
          ...
          {['busy', 'notfound', 'error'].includes(status) && (
            <CentreBox>
              <div style={{ backgroundColor: '#1E293B', border: '1px solid #111827', borderRadius: 12, padding: '24px 28px', maxWidth: 360, textAlign: 'center' }}>
                <h3 style={{ color: '#111827', fontSize: 15, fontWeight: 700, margin: '0 0 8px' }}>
          ...
          {isReady && (
            <div style={{ position: 'absolute', bottom: 16, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 10 }}>
              {[{ l: `Mic ${micOn ? 'On' : 'Off'}`, a: micOn }, { l: `Cam ${camOn ? 'On' : 'Off'}`, a: camOn }].map(b => (
                <span key={b.l} style={{ padding: '5px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, backgroundColor: b.a ? 'rgba(22,163,74,0.18)' : 'rgba(220,38,38,0.18)', color: b.a ? '#111827' : '#111827', border: `1px solid ${b.a ? '#111827' : '#111827'}` }}>
                  {b.l}
                </span>
              ))}
            </div>
          ...
          {!isReady && (
            <div style={{ padding: '16px 20px', backgroundColor: '#F8FAFC', borderBottom: '1px solid var(--border-color)' }}>
              <button
                onClick={startStream}
                style={{
                  width: '100%', padding: '11px', backgroundColor: '#475569', color: '#FFFFFF',
            ...
            {[
              ...
              <div key={d.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', borderRadius: 8, backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', marginBottom: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: isReady ? (d.active ? '#111827' : '#111827') : '#9CA3AF', fontWeight: 500 }}>
